...
# 本文の文字数の目安。公開済みの記事3本の実測から決めている。
#   記事01 = 4,472字 / 記事02 = 4,972字 / 記事03 = 3,508字
# 地図型(03)は短く、書評(01/02)は長い。両方が収まる幅を、下限側に余裕を
# 持たせて取る(3,508 に対して 3,500 では、少し削っただけで警告が出る)。
# 記事が増えたら測り直す。**警告を消すために本文を削らない** — 値のほうを直す。
TARGET_MIN_CHARS: int | None = 3300
TARGET_MAX_CHARS: int | None = 5500
...
IDENTIFYING = [
    ...
    (
        re.compile(r"特許|発明者"),
        "書かないと決めている。出願公開で発明者名が公開されるため、実名への経路になる",
    ),
    (
        re.compile(r"治験"),
        "中身は書かない方針。語そのものも施設を絞る材料になるので、"
        "「研究が動いている環境」で足りないか確かめる",
    ),
]
