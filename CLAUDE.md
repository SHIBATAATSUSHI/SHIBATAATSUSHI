...
- `claude` の起動を案内するときは**必ず `cd` から書く**。作業ディレクトリは起動時に固定され、あとから `cd` しても変わらない。省いたために `/codex:review` が「git リポジトリでない」で2回失敗した。
- 環境依存のバグは実機でしか出ない。クラウドのコンテナには端末が無いため `isatty()` が偽になり、対話プロンプトで停止するテストが素通りしていた。「こちらで通ったから大丈夫」とは言わず、実機での実行結果を確かめる。
