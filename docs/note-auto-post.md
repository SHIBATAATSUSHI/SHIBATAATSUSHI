...
入力欄も編集領域も取れず、テキストの無いボタンが数個残るだけになることがある。
このときセレクタは全滅するが、**原因はセレクタではない。画面が完成していない。**
「note のUIが変わった」と読み違えて定義を書き換えると、当たっていたものまで壊す。

`doctor` はこの状態を検出して、レポート冒頭に「**この採取は使えない**」と出す。
見る順番は次のとおり。

1. **読み込み時のエラー** — JS の例外、コンソール、400以上のレスポンス。
   外枠だけ出て中身が組み上がらない原因は、たいていここにある
2. **別の下書きURLで同じ症状が出るか** — 出なければその記事だけの問題、
   出れば note 側かこの環境の問題。切り分けの費用が一番安い

```bash
python scripts/note_post.py doctor --url <別の下書きURL> --headed --keep-open
```

`--keep-open` を付けると採取後もブラウザが開いたままになる。開発者ツールを開いて
自分の目で確かめられる。
...
- 実機で下書き保存に成功した実績は `--headed` での実行のみ。ヘッドレスは未確認。
